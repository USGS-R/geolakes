...
# flipping so that characteristic types are on the y axis and stacked bars are the site types

data_bychar <- read.csv("inst/extdata/wqp_sites_records_groupedby_chartype.csv")

data_sites_bychar <- data_bychar %>%  
  mutate(percentSites = (numSites/totalNumUniqueSites)*100) %>% 
  mutate(charTypeLabels = paste0(characteristicType, "\n (", totalNumUniqueSites, ")"))

site_cols <- c('#a6cee3','#1f78b4','#b2df8a','#33a02c','#fb9a99','#e31a1c',
               '#fdbf6f','#ff7f00','#cab2d6','#6a3d9a','#ffff99','#bebada',
               '#fb8072','#80b1d3','#fdb462')

ggplot(data_sites_bychar, aes(x = charTypeLabels, 
                              y = percentSites, 
                              fill = siteType)) + 
  geom_bar(stat="identity") + 
  coord_flip() + 
